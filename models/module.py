...
class GAT(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        for att in self.attentions:
            x_temp, att_temp = att(x, adj)
            atts.append(att_temp)
            x_temps.append(x_temp)

        x = torch.cat(x_temps, dim=2)
        # if self.nlayers > 2:
        #     for i in range(self.nlayers - 2):
        #         temp = []
        #         x = F.dropout(x, self.dropout, training=self.training)
        ...
        x = F.dropout(x, self.dropout, training=self.training)
        out_att, attention = self.out_att(x, adj)
        x = F.elu(out_att)
        return x + input, x
...
class ModelManager(nn.Module):
    ...
    def __init__(self, args, num_word, num_slot, num_intent):
        ...
        self.__intent_embedding = nn.Parameter(
            torch.FloatTensor(self.__num_intent, self.__args.intent_embedding_dim))  # 7, 64
    ...
    def generate_adj_gat(self, index, batch):
        ...
        ''' tensor([[0, 2],
                    ...
                    [2, 4],
                    [2, 6]])), '''
        # batch = len(pred_intent) = 64
        intent_idx_ = [[torch.tensor(0)] for i in range(batch)]
        #[[tensor(0)],[tensor(0)],[tensor(0)]]
        for item in index: # item = tensor([0, 2])
    ...
    def forward(self, text, seq_lens, n_predicts=None, forced_slot=None, forced_intent=None):
        ...
        pred_intent = self.__intent_decoder(g_c) # (64, 7)
        ...
        pred_slot, slot_intent_cor = self.__slot_decoder(
            g_hiddens,
            seq_lens,
        ...
        embedding_intents = self.__intent_embedding.unsqueeze(0)
        slot_intent_cor = slot_intent_cor.transpose(2, 1)
        # 18, 1, 64 x 1, 7, 64
        slot_intent_cor = embedding_intents.matmul(slot_intent_cor)
        ...
        if n_predicts is None:
            ...
            return slot_index.cpu().data.numpy().tolist(), intent_index.cpu().data.numpy().tolist(), slot_intent_cor
...
class LSTMDecoder(nn.Module):
    ...
    def __init__(self, args, input_dim, hidden_dim, output_dim, dropout_rate, embedding_dim=None, extra_dim=None):
        ...
        args,
        input_dim = self.__args.encoder_hidden_dim + self.__args.attention_output_dim,
    ...
    def forward(self, encoded_hiddens, seq_lens, forced_input=None,  # extra_input=None,
        ...
        input_tensor = encoded_hiddens # = g_hidden (64, 37, 384)
        ...
        if self.__embedding_dim is not None and forced_input is not None:
            ...
            for word_i in range(seq_lens[0]):
                ...
                # input_tensor.shape: (1, 17, 384)
                # input_tensor[:, word_i].unsqueeez: (1, 1, 384)
                # prev_tensor: (1, 1, 32)
                # "combine input:" (1, 1, 416)
                dropout_input = self.__dropout_layer(combined_input)
                ...
                if adj is not None:
                    ...
                    lstm_out_origin, attention = self.__graph(lstm_out, adj)
                    lstm_out = lstm_out_origin[:, 0]
                intent_slot_cor.append(lstm_out) # corresponse intent and slot
        ...
        return torch.cat(output_tensor_list, dim=0), torch.stack(intent_slot_cor)
...
class UnflatSelfAttention(nn.Module):
    ...
    def forward(self, inp, lens): # Encode the text to Encoded SelfAttention Text
        batch_size, seq_len, d_feat = inp.size() # (64, 37, 384)
        inp = self.dropout(inp)
        scores = self.scorer(inp.contiguous().view(-1, d_feat)).view(batch_size, seq_len) # (64, 37) (batch_size, sequence_length)
        max_len = max(lens)
        for i, l in enumerate(lens):
            if l < max_len:
                scores.data[i, l:] = -np.inf # Mask the sequence to the right length
